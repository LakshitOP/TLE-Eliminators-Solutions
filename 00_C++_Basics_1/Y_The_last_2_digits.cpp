...
void solve() {
    ll a,b,c,d;
    cin >> a >> b >> c >> d;
    a = a%100; // Keep only last 2 digits
    b = b%100; // Keep only last 2 digits
    c = c%100; // Keep only last 2 digits
    d = d%100; // Keep only last 2 digits
    ll x = a*b*c*d;
    x=x%100; // Keep only last 2 digits again
